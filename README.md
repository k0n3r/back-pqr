...
Módulo para la gestión integral de Peticiones, Quejas y Reclamos (PQR) dentro del sistema SAIA. Permite configurar formularios dinámicos, publicarlos como webservice público, gestionar el ciclo de vida de cada PQR y responderla con trazabilidad completa.
...
La integración IA se carga condicionalmente: solo si el bundle `ia` está instalado. Los archivos de configuración condicional están en `Resources/config/ia_routes.php` e `ia_services.php`.

### 11.1 Chat IA

Endpoint: `POST /api/pqr/ia/chat`

Permite a funcionarios hacer preguntas en lenguaje natural sobre PQRs. Extiende `AbstractModuleChatController` del bundle IA con el contexto específico de PQR.

```json
{
  "message": "¿Cuántas PQRs de tipo tutela hay pendientes este mes?",
...
### 11.2 Tools para el agente IA

**`PqrTool`** (`create_response_pqr`) — permite al agente registrar una respuesta oficial a una PQR. Recibe `documentId`, `subject` y `contentAnswers`, valida que la PQR tenga IA habilitada (`PqrIaGuard`), y crea el documento de respuesta con los datos por defecto del funcionario activo.

**`PqrStatsTool`** — estadísticas y conteos disponibles como tools del agente:

| Tool | Descripción |
|---|---|
| `get_pqr_available_filters` | Lista los campos disponibles para filtrar |
| `get_pqr_filter_options` | Opciones de `sys_dependencia` y `sys_tipo` |
| `count_pqr` | Cuenta PQRs aplicando filtros y agrupaciones |

Los campos soportados en `count_pqr` son: `fecha`, `canal_recepcion`, `sys_estado`, `sys_fecha_vencimiento`, `sys_fecha_terminado`, `sys_tipo`, `sys_dependencia`. Consulta la vista `vpqr`.

### 11.3 Servidor MCP

**`PqrMcpFormatProvider`** implementa `McpFormatProviderInterface` y registra el formato PQR en el servidor MCP bajo el tag `saia.mcp.format_provider`. El `idformato` se obtiene dinámicamente desde `PqrForm::getInstance()->fk_formato`.

Esto permite que agentes externos (Claude Desktop, Cursor, AWS AgentCore) radiquen PQRs autónomamente usando las tools del servidor MCP:
...
### 11.4 Procesador de formato para IA (`PqrFormatoJsonProcessor`)

Personaliza el JSON de estructura del formato PQR que se entrega al agente. Implementa `FormatoJsonProcessorInterface`:

- **Excluye** campos internos: `destino_interno`, `colilla`, `select_mensajeria`.
- **Agrega** los campos de tipo `TYPE_DEPENDENCIA` y `TYPE_LOCALIDAD` definidos en el formulario como campos de lookup (tipo `City`), con descriptor `search_field_options` para MCP o endpoint REST para clientes HTTP.
- Los metadatos de lookup se **cachean por 1 semana** (clave `pqr_ia_lookup_fields`) para evitar consultas repetidas a BD.

Para invalidar el caché si se modifican campos del formulario:
```bash
php bin/console cache:pool:delete cache.app pqr_ia_lookup_fields
```

### 11.5 Serialización para Knowledge Base

`PqrJsonForIA` y `PqrRespuestaDataJsonForIA` convierten documentos PQR y sus respuestas a JSON/Markdown para ser indexados en AWS Bedrock Knowledge Base por el comando `app:ia:full-sync`.
...
Al publicar el formulario (`PUT /api/pqr/form/publish`), el sistema genera automáticamente los archivos necesarios para el webservice:

**URL pública:** `https://DOMINIO/ws/pqr`

El ciudadano accede, completa el formulario con los campos configurados y su PQR queda radicada en SAIA. No requiere autenticación.
...
La clase `Services/Customizable/PqrCustomizable.php` es la base extensible del módulo. Permite agregar funcionalidad específica por cliente sin modificar el core del bundle. Extiende esta clase para sobreescribir comportamientos puntuales del ciclo de vida de la PQR.
